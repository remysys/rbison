...
static int Shite_reduce   = 0;  /* number of shift/reduce conflicts */
...
static void add_goto(int state, int nonterminal, int go_here)
{
  ...
  if (p = p_goto(state, input_sym)) {
    error(FATAL, "tried to add duplicate goto on nonterminal %s\n"
                 ...
                 p->go_here, p->state, go_here, state);
  }
...
static unsigned int state_hash(STATE *sym)
{
  ...
  for (; --nitems >= 0; ++items) {
    total += (*item)->prod_num + (*item)->dot_posn; 
  }
...
static int item_cmp(ITEM **item1p, ITEM **item2p)
{
...
static int lr(STATE *cur_state)
{
  ...
  while (cur_state = get_unfinished()) {
    ...
    if (nclose == 0) {
      ...
      qsort((void **)closure_items, nclose, sizeof(ITEM), (int (*)(void*, void*)) item_cmp);
...
staitc int kclose(STATE *kernel, ITEM **closure_items, int maxitems, int nclose)
{
...
static void reduce_one_item(ITEM *item, STATE *state)
{
